...
class FishPond {
  constructor(window){
    ...
    this.ripples=[];
    this.fish=[];
    let fishCount = this.height * this.width / 50000;
  ...
  render(ctx){
    ...
    ctx.font = `${this.fontSize*.7}px Arial`;
      ctx.fillStyle='blue';
      ctx.fillRect(this.fontSize*3,this.fontSize*3,this.fontSize*5.5,this.fontSize*1.1);
      ctx.fillStyle='white';
      ctx.fillRect(this.fontSize*3.5,this.fontSize*3.5,this.fontSize*5.5,this.fontSize);
      ctx.fillStyle='black';
      ctx.fillText('Attract More Fish',this.fontSize*4,this.fontSize*4);
    }
  }
  click(x,y){
    this.instructions=false;
    let food = new Target(x,y,3);
    if(this.opacity < 0 &&x > this.fontSize*3 && y>this.fontSize*3&&
        x<this.fontSize*5.5 && y<this.fontSize*1.1){
      this.addFish();
      this.var+=0.001
    }
    else{
