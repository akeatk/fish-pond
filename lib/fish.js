...
class Tail{
  ...
  render(ctx){
    this.act();
    ctx.fillStyle=`rgba(${this.fish.colors[this.fish.baseColor][0]},${this.fish.colors[this.fish.baseColor][1]},${this.fish.colors[this.fish.baseColor][2]},1)`;
...
class TailPiece{
  ...
  act(tail){
    ...
    this.y += this.vely;
    this.updateRadian();
    let angleDiff = this.radian+this.offset/4 - this.prevPart.radian;
    if(angleDiff > Math.PI)
      angleDiff -= Math.PI * 2;
    else if(angleDiff < -Math.PI)
      angleDiff += Math.PI * 2;
...
class Fin{
  constructor(fish,side,ratio,radius){//1 is left, -1 is right
    ...
    this.pieceLength = this.radius * 2 * ratio;
    this.pieces = [new FinPiece(this,undefined,this.side)];
    for(let i = 0;i < 5;i++)
      this.pieces.push(new FinPiece(this,this.pieces[i],this.side));
  }
  updateRadius(radius){
    this.radius = radius;
  ...
  render(ctx){
    ...
    // for(let j = 0;j < this.pieces.length;j++){
      ...
      for(let i = 1;i < this.pieces.length;i++){
        ctx.lineTo(this.pieces[i].x,this.pieces[i].y);
            console.log('fin');//figure out fins
      }
      ctx.stroke();
      ctx.closePath();
    // }
    ...
    // ctx.beginPath();
    // ctx.arc(this.pieces[0].x,this.pieces[0].y,10,0,2*Math.PI);
    // ctx.fill();
    // ctx.closePath();
  }
}
class FinPiece{
  constructor(fin,prevPart,side){
    this.fin = fin;
    this.prevPart = prevPart;
    this.side = side;

    if(!this.prevPart){
      [this.x,this.y] = this.fin.part.getPoint(this.fin.part.radius*8,this.side*Math.PI/2);
      this.maxAngle = this.fin.part.maxAngle;
      this.radian = this.fin.part.radian + Math.Pi / 2 * this.side;
    }
    else{
      this.maxAngle = this.prevPart.maxAngle;
      this.radian=this.prevPart.maxAngle;
      [this.x,this.y] = this.prevPart.getPoint(this.fin.pieceLength,this.prevPart.radian+Math.PI);
    }
  }
  act(){
    if(!this.prevPart){
      ...
      if(Math.abs(angleDiff) > this.maxAngle){//angle correction
        if(angleDiff > 0){
          if(angleDiff > this.maxAngle)
            [this.x,this.y] = this.prevPart.getPoint(this.fin.pieceLength,this.prevPart.radian+this.maxAngle)
          else{
            this.x = this.prevPart.x + this.segmentLength * Math.cos(this.radian + Math.PI);
            this.y = this.prevPart.y + this.segmentLength * Math.sin(this.radian + Math.PI);
          }
